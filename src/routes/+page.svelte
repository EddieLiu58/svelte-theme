<script lang="ts">
	...
	async function fetchList() {
		...
		// 玄幻
		let fantasy = await axios.get(`${base}/api/novels/category?page=1&size=12&cat=玄幻`);
		fantasylList = [...fantasylList, ...fantasy.data];
		// 科幻
		let science = await axios.get(`${base}/api/novels/category?page=1&size=12&cat=科幻`);
		scienceList = [...scienceList, ...science.data];
		// 校園
		let school = await axios.get(`${base}/api/novels/category?page=1&size=12&cat=校園`);
		schoolList = [...schoolList, ...school.data];
		// 青春
		let youth = await axios.get(`${base}/api/novels/category?page=1&size=12&cat=青春`);
