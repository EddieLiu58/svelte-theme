...
<div class="w-full bg-[url('./images/blob.png')] bg-cover bg-no-repeat">
  <div class="w-full backdrop-blur-lg">
    <div class="grid grid-cols-2 gap-12  max-w-3xl w-full min-h-[500px] py-16 mx-auto">
      <div class="overflow-hidden rounded-md">
        <img src="{baseUrl}/images/{id}.jpg" class="w-auto" alt="" />
      </div>
      <div class="flex flex-col  justify-between text-white">
        <div class="grid gap-4">
          <h2 class=" text-4xl">{ item.name?item.name:'' }</h2>
          <span class="text-lg">作者: { item.author?item.author:'' }</span>
          <span class="text-lg">分類: { item.category?item.category:'' }</span>
          <p class="text-lg">{ item.introduction?item.introduction:'' }</p>
        </div>
        <div class="w-full grid grid-cols-2 gap-4">
          <a href="/{id}/{firstId}" class="w-full border-2 border-solid  border-white p-4 text-center rounded-[180px] hover:bg-white transition-all hover:text-gray-400">開始閱讀</a>
          <a href="/list" class="w-full border-2 border-solid  border-white p-4 text-center rounded-[180px] hover:bg-white transition-all hover:text-gray-400">返回列表頁</a>
        </div>
...
<div class="flex flex-col max-w-4xl px-4 mt-9 mx-auto w-full gap-4">
  ...
  <div class="grid grid-cols-1 gap-4">
    ...
    <a href="/{id}/{item.id}" class="flex gap-2">
      { item.title }
    </a>
  {/each}
  </div>
